...
def _process_routes():
    """返回 app.services.process_service 模块（供 monkeypatch 进程记录函数）。

    注：create_task_process / complete_task_process 已从 api 层下沉到
    app.services.process_service（消除下层对 api 层的反向依赖），桩位点随之迁移。
    """
    return __import__("app.services.process_service", fromlist=["x"])
...
class TestHolidayDailySummary:
    ...
    def test_different_type_separate_summary(self, holiday, monkeypatch, session_factory):
        ...
        try:
            assert (
                s.query(TaskProcess)
                .filter_by(name=_HOLIDAY_SUMMARY_NAME, task_type="weather")
                .count()
                == 1
            )
            assert (
                s.query(TaskProcess)
                .filter_by(name=_HOLIDAY_SUMMARY_NAME, task_type="electricity")
                .count()
                == 1
            )
        finally:
    ...
    def test_record_true_not_in_summary(self, holiday, monkeypatch, session_factory):
        ...
        holiday._add_period()
        pr = _process_routes()
        monkeypatch.setattr(pr, "create_task_process", lambda *a, **k: 1)
        ...
        s = session_factory()
        try:
            assert s.query(TaskProcess).filter_by(name=_HOLIDAY_SUMMARY_NAME).count() == 0
        finally:
            s.close()
...
class TestTaskEntryGates:
    ...
    def test_entry_gate_short_circuits_when_holiday_active(
        self, monkeypatch, module_path, func_name
    ):
        ...
        monkeypatch.setattr(
            HS, "skip_if_active", lambda name, task_type="generic", record=True: True
        )
