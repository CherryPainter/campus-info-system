...
class TaskService:
    ...
    def _cleanup_delivered(self):
        ...
        with self._lock:
            ...
            # 只保留今天的记录
            self._delivered = set(
                (rid, sid, d) for rid, sid, d in self._delivered if d == today
            )
        ...
        try:
            ...
            try:
                ...
                row = PushTask(
                    ...
                    rule_id=str(task_data.get('rule_id')) if task_data.get('rule_id') is not None else None,
                    schedule_id=str(task_data.get('schedule_id')) if task_data.get('schedule_id') is not None else None,
