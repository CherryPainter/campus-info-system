...
from app.utils.course_helpers import (
    PERIOD_TIME_MAP,
    get_current_period_and_status,
    get_current_week_number,
    get_schedule_by_building,
    is_course_in_week,
)
        ...
        for c in courses:
            ...
            if key in merged:
                ...
                if isinstance(existing_periods_list, str):
                    existing_periods_list = [
                        int(p) for p in existing_periods_list.split(",") if p.strip().isdigit()
                    ]
                ...
                if isinstance(new_periods_list, str):
                    new_periods_list = [
                        int(p) for p in new_periods_list.split(",") if p.strip().isdigit()
                    ]
...
def get_timetable():
    ...
    # 学期过滤：未显式指定时默认取当前学期（保证与历史行为一致）
        ...
        # 周次列表：基于开学日推算生成（不再查 course_weeks 表，杜绝脏数据）
        ...
        if week_number is None:
            if is_current_semester:
                # 本学期：使用真实当前周；非教学周返回 0 时回退到第 1 周
        ...
        # 教学周 / 假期判断：管理员激活假期模式、或当前不在教学周时，
        # 强制 is_ongoing=False，避免暑假/假期期间前端仍显示"上课中 - 第X节"标签。
        # 权威顺序：假期模式（holiday_service，管理员显式配置）> 教学周（course_weeks 派生）。
        try:
            from app.services.holiday_service import holiday_service

            holiday_active, _ = holiday_service.is_active()
        except Exception:
            holiday_active = False
        try:
            in_teaching = _is_in_teaching_week()
        except Exception:
            in_teaching = True  # 异常时回退 True，不因判断失败而阻断
        is_teaching = in_teaching and not holiday_active
        current_status["is_teaching_week"] = is_teaching
        if not is_teaching:
            current_status["is_ongoing"] = False
            ...
            if is_current_semester and c.week_day == current_week_day:
                ...
                if isinstance(periods_data, list):
                    course_periods = [
                        ...
                        if isinstance(p, int | str) and str(p).strip().isdigit()
                    ]
                elif isinstance(periods_data, str):
                    course_periods = [
                        int(p) for p in periods_data.split(",") if p.strip().isdigit()
                    ]
                else:
...
def create_or_update_course():
    ...
    try:
        ...
        if is_edit and old_course_id:
            ...
            [c.id for c in old_courses]
            old_periods = {c.period_idx for c in old_courses}
            ...
            logger.info(
                f'[课程] 更新课程: {data["course_name"]} (更新{len(updated_courses)}条, 新增{len(added_courses)}条, 删除{len(to_delete)}条, 推送: {push_enabled and "开启" or "关闭"})'
            )
        ...
        return api_success(
            message=f"成功创建 {len(created_courses)} 条课程记录",
            data=[c.to_dict() for c in created_courses],
        )
    finally:
        session.close()
        ...
        if "periods" in data:
            ...
            if isinstance(periods_raw, list):
                ...
                if periods_raw.strip():
                    course.periods = [
                        int(x.strip()) for x in periods_raw.split(",") if x.strip().isdigit()
                    ]
                else:
                    course.periods = []
        if "weeks" in data:
            ...
            if isinstance(weeks_raw, list):
                ...
                if weeks_raw.strip():
                    course.weeks = [
                        int(x.strip()) for x in weeks_raw.split(",") if x.strip().isdigit()
                    ]
                else:
                    course.weeks = []
    ...
    try:
        ...
        for course_data in courses_data:
            ...
            def parse_cn_num(cn_str: str, mapping=cn_num_map) -> int:
                """解析中文数字（mapping 默认参数绑定，避免闭包 late-binding）"""
    ...
    try:
        ...
        if task_dict["schedule_type"] == "immediate":
            ...
            thread = threading.Thread(
                target=crawl_svc._run_scheduled_crawl, args=(task_dict["id"],), daemon=True
            )
            thread.start()
...
def list_crawl_tasks():
    ...
    try:
        ...
        try:
            ...
            total = query.count()
            tasks = (
                query.order_by(ScheduledCrawlTask.created_at.desc())
    ...
    try:
        ...
        try:
            ...
            if task.status in ("completed", "failed", "cancelled"):
                return api_error(
                    message=f"任务已处于 {task.status} 状态，无法取消", http_status=400
                )
            task.status = "cancelled"
            task.completed_at = datetime.now()
            task.message = "用户取消"
...
def run_crawl_task_now(task_id):
    ...
    try:
        ...
        thread = threading.Thread(
            target=crawl_svc._run_scheduled_crawl, args=(task_id,), daemon=True
        )
        thread.start()
