...
export default function WeatherChart() {
  ...
  const tempPopOption = {
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        params.forEach((p: any) => { s += `${p.marker} ${p.seriesName}: ${p.value}${p.seriesName === '温度' ? '°C' : '%'}`; });
        return s;
      },
    },
    ...
    yAxis: [
      { type: 'value' as const, name: '温度(°C)', nameTextStyle: { fontSize: 11 }, min: Math.min(...temps) - 2, max: Math.max(...temps) + 2 },
      { type: 'value' as const, name: '降水概率(%)', nameTextStyle: { fontSize: 11 }, max: 100, splitLine: { show: false } },
    ],
    series: [
      {
        name: '温度',
  ...
  const humidityOption = {
    tooltip: { trigger: 'axis' as const, formatter: (params: any) => `${params[0].axisValue}<br/>湿度: ${params[0].value}%` },
    grid: { left: 50, right: 20, top: 30, bottom: 30 },
    xAxis: { type: 'category' as const, data: hours, axisLabel: { fontSize: 11, rotate: hours.length > 12 ? 45 : 0 } },
    yAxis: { type: 'value' as const, name: '湿度(%)', nameTextStyle: { fontSize: 11 }, min: 0, max: 100 },
    series: [{
      name: '湿度',
      type: 'line',
      data: humidities,
  ...
  /** 风力信息 */
  const windInfo = hourly.map(h => ({ time: hours[hourly.indexOf(h)], text: h.text, wind: `${h.wind_dir} ${(h as any).wind_scale || (h as any).windScale}级` }));
    ...
    <div>
      ...
      {now && (
        <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
          ...
          <Col xs={12} sm={6}>
            <Card size="small" styles={{ body: { height: 90, display: 'flex', flexDirection: 'column', justifyContent: 'center' } }}>
              <Statistic title="当前温度" value={now.temp} suffix="°C" prefix={<FireOutlined />} valueStyle={{ color: '#ff6b35' }} />
              <div style={{ color: '#999', fontSize: 12, marginTop: 4 }}>体感 {now.feels_like}°C</div>
            </Card>
          </Col>
          <Col xs={12} sm={6}>
            <Card size="small" styles={{ body: { height: 90, display: 'flex', flexDirection: 'column', justifyContent: 'center' } }}>
              <Statistic title="相对湿度" value={now.humidity} suffix="%" prefix={<CloudOutlined />} valueStyle={{ color: '#52c41a' }} />
            </Card>
          </Col>
          <Col xs={12} sm={6}>
            <Card size="small" styles={{ body: { height: 90, display: 'flex', flexDirection: 'column', justifyContent: 'center' } }}>
              <Statistic title="风向风力" value={`${now.wind_dir} ${now.wind_scale}级`} prefix={<FlagOutlined />} valueStyle={{ fontSize: 16 }} />
            ...
            <div style={{ maxHeight: 280, overflowY: 'auto' }}>
              {windInfo.map((w, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid #f0f0f0' }}>
                  ...
                  <Tag color="blue" style={{ width: 110, textAlign: 'center', justifyContent: 'center' }}>{w.wind}</Tag>
                </div>
              ))}
            </div>
          </Card>
