...
class ElectricityRemaining(Base):
    ...
    def __repr__(self) -> str:
        return (
            f"<ElectricityRemaining(id={self.id}, meter={self.meter}, remaining={self.remaining})>"
        )
...
class ElectricityTotalCapacity(Base):
    ...
    record_reason = Column(
        String(50),
        nullable=False,
        default="auto_detect",
        comment="记录原因: auto_detect-自动检测, low_power-低电量警告, manual-手动设置",
    )
    recorded_at = Column(DateTime, nullable=False, default=datetime.utcnow, comment="记录时间")
    created_at = Column(DateTime, nullable=False, default=datetime.utcnow, comment="创建时间")

    # 复合索引：按电表和时间查询
    __table_args__ = (Index("idx_capacity_meter_time", "meter", "recorded_at"),)
