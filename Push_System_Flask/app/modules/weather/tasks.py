...
def _make_analyzer():
    ...
    state_dir = getattr(Config, 'QWEATHER_DATA_DIR',
                        os.path.join(Config.BASE_DIR, 'data', 'weather'))
...
def _update_weather_alert_impl() -> None:
    ...
    try:
        ...
        if data:
            try:
                ...
                try:
                    ...
                    try:
                        from app.services.config_service import get_config_service
                        _city_name = get_config_service().get('weather', 'city_name', '重庆') or '重庆'
                    except Exception:
                        _city_name = '重庆'
                    ...
                    for alert in data:
                        ...
                        if end_dt is not None:
                            try:
                                if now_dt > end_dt:
                                    logger.info(f'[天气] 预警 {alert_id} 已过期({end_time_str})，跳过入库')
                                    continue
                            except (ValueError, TypeError):
                                pass

                        # 存入数据库（修正参数签名，匹配 WeatherRepository.create_alert）
                        _severity = alert.get('severity', '') or alert.get('color_code', '') or 'unknown'
                        _color_code = alert.get('color_code', '') or alert.get('severity', '') or 'unknown'
                    ...
                    for aa in active_alerts:
                        if aa.alert_id not in current_ids:
                            ...
                            logger.info(f'[天气] 预警已自动解除: {aa.alert_id} - {(aa.headline or "")}')
        ...
        complete_task_process(pid, 'completed', f'天气预警更新完成，{len(data) if data else 0} 条预警')
    except Exception as exc:
...
def _check_and_push_new_alerts(alerts: list, cache) -> None:
    ...
    try:
        from app.services.config_service import get_config_service
        alert_enabled = str(get_config_service().get('weather', 'alert_enabled', 'true')).lower() not in ('false', '0', 'no', 'off')
    except Exception:
        alert_enabled = True
...
def push_weather_analysis() -> None:
    ...
    try:
        ...
        logger.info(f'[天气] 缓存状态: 实时天气={bool(now_data)}, 逐小时预报={bool(hourly_data)}, 预警信息={bool(alert_cache)}')
        ...
        if not now_data:
            ...
            if now_data:
                cache.set('now', now_data, _CACHE_TTL_NOW)
                logger.info(f'[天气] 实时天气重新获取成功: {now_data.get("text")}, 温度={now_data.get("temp")}')
            else:
        ...
        logger.info(f'[天气] 当前天气: {current_weather}, 温度={current_temp}, 体感={current_feels_like}')
        ...
        if parts:
            ...
            if _maybe_push(combined):
                ...
                logger.info(f'[天气] 分析汇总被抑制（免打扰/达上限），合并 {len(parts)} 个事件未推送')
        else:
...
def register_tasks(scheduler, app) -> None:
    ...
    from app.services.config_service import get_config_service
    config_svc = get_config_service()
    daily_time = config_svc.get('weather', 'schedule_daily', None) or app.config.get('WEATHER_SCHEDULE_DAILY', '07:00')
    try:
