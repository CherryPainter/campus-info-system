...
export default function Cookies() {
  ...
  const columns = [
    { title: "Cookie 名称", dataIndex: "name", key: "name", width: 160 },
    {
      ...
      key: "type",
      width: 80,
      render: (v: string) => <Tag color={v === "必要" ? "blue" : "green"}>{v}</Tag>,
    },
  ...
  return (
    <div style={{ maxWidth: 900, margin: "40px auto", padding: "0 24px" }}>
      <Card bordered={false} style={{ borderRadius: 12, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
        ...
        <Paragraph>
          Cookie 是网站在您访问时存储在您的计算机或移动设备上的小型文本文件。 Cookie
          被广泛用于使网站正常运行、提高运行效率以及向网站所有者提供信息。 除了 Cookie
          外，我们还可能使用 localStorage（本地存储）来实现类似的功能。
        </Paragraph>

        <Title level={4}>二、我们使用的 Cookie</Title>
        <Paragraph>
          本系统仅使用必要的 httpOnly Cookie 来提供核心功能。这些 Cookie 由后端服务设置，前端
          JavaScript
          代码无法读取或修改，有效防止跨站脚本攻击窃取令牌。本系统不包含广告追踪或第三方分析
          Cookie：
        </Paragraph>
        ...
        <Paragraph style={{ fontSize: 15, lineHeight: 2, textIndent: "2em" }}>
          ...
          <Text strong>勾选</Text>时为较长的长期会话。 无论哪种情况，关闭浏览器本身不会清空 httpOnly
          Cookie，但会话仍会按上述时长到期，到期后将自动退出登录。
        </Paragraph>
        <Paragraph>
          <ul>
            <li>
        ...
        <Paragraph>
          本系统<Text strong>不使用任何第三方 Cookie</Text>
          （如广告追踪器、社交媒体插件、分析服务等）。 所有 Cookie
          均由本系统自身设置，数据完全由我们控制，不会被任何第三方访问。
        </Paragraph>
        ...
        <Paragraph>
          除 httpOnly Cookie 外，本系统的前端界面状态（如侧边栏折叠、页面切换等）完全由 React
          组件状态管理， 不会持久化到浏览器本地存储。登出时，系统会清理任何历史遗留的本地存储数据。
        </Paragraph>

        <Title level={4}>七、数据安全</Title>
        <Paragraph>存储在 Cookie 中的身份令牌经过签名保护，可防止被篡改。 我们建议您：</Paragraph>
        <Paragraph>
          <ul>
            <li>使用完毕后及时退出登录，清除会话 Cookie；</li>
            <li>不要在公共或共享设备上选择"记住我"功能；</li>
            <li>
              头像上传仅支持 JPG / PNG / GIF / WEBP 且不超过 2MB，请勿上传来源不明或异常格式的文件；
            </li>
