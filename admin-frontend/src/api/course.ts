...
export const courseApi = {
  ...
  /** 删除课程（硬删除，从数据库中完全删除） */
  delete: (id: number) =>
    request.delete<any, ApiResponse>(`/course/${id}`),
  ...
  /** 从爬虫数据导入课程 */
  import: () =>
    request.post<any, ApiResponse<{ imported_count: number }>>('/course/import', {}),

  /** 获取学期列表 */
  getSemesters: () =>
    request.get<any, ApiResponse<{
      semesters: { id: number | string; name: string; eams_id?: number | string; is_current?: boolean }[];
  ...
  crawlTasks: {
    /** 列表 */
    list: async (params?: { status?: string; scope?: string; page?: number; per_page?: number }) => {
      const res = await request.get<any, ApiResponse<CrawlTask[]> & { pagination: { total: number; page: number; per_page: number; pages: number } }>('/course/crawl-tasks', { params });
    ...
    /** 详情 / 状态轮询 */
    get: (id: number) =>
      request.get<any, ApiResponse<CrawlTask>>(`/course/crawl-tasks/${id}`),
    ...
    /** 删除 */
    delete: (id: number) =>
      request.delete<any, ApiResponse>(`/course/crawl-tasks/${id}`),
    /** 取消（pending/running -> cancelled） */
    cancel: (id: number) =>
      request.post<any, ApiResponse>(`/course/crawl-tasks/${id}/cancel`),
    /** 立即执行一个 pending 任务 */
    run: (id: number) =>
      request.post<any, ApiResponse>(`/course/crawl-tasks/${id}/run`),
  },
};
