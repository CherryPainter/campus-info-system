...
class JWTManager:
    ...
    def generate_tokens(
        ...
        absolute_expire = (
            absolute_expire if absolute_expire is not None else self.refresh_absolute_expire
        )
        now = time.time()
    ...
    def refresh_access_token(self, refresh_token):
        ...
        if not role:
            logger.error(
                f'JWT refresh 失败: token payload 缺少 role 字段 (user={payload.get("username")})'
            )
    ...
    def cleanup_expired_blacklist(self, max_age=86400):
        ...
        try:
            ...
            deleted = (
                session.query(TokenBlacklist).filter(TokenBlacklist.revoked_at < cutoff).delete()
            )
            session.commit()
            if deleted > 0:
