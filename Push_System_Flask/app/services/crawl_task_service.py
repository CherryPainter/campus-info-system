...
def _crawl_one_semester(
    eams_id: str, semester_id: int, week: int = None, timeout: int = 900, scope_label: str = None
):
    """
    爬取单个学期并导入数据库。
    ...
    processed_subdir = os.path.join(
        spider_dir, "output", "course-data", "processed", f"semester_{eams_id}"
    )
    ...
    # 系统侧接管落库（爬虫子进程只产出 JSON/图片）。
    # 明细进程（课程全量爬取·学期X / ·全部学期）由系统创建并标记，与伞进程互补。
    from app.services.process_service import complete_task_process, create_task_process

    if scope_label:
        _detail_name = f"课程全量爬取·{scope_label}"
    elif semester_id:
        _detail_name = f"课程全量爬取·学期{semester_id}"
    else:
        _detail_name = "课程全量爬取（未指定学期）"
    detail_pid = None
    try:
        detail_pid = create_task_process(
            _detail_name, "course_full_crawl", total_items=1, created_by="system"
        )
    except Exception as pe:
        logger.warning(f"[爬取任务] 创建明细进程失败（不影响导入）: {pe}")
    ...
    if detail_pid is not None:
        if created < 0:
            complete_task_process(detail_pid, "failed", error="数据导入失败")
        elif created == 0 and updated == 0:
            complete_task_process(detail_pid, "completed_empty", "未导入任何课程（可能无排课）")
        else:
            complete_task_process(
                detail_pid, "completed", f"成功导入 {created} 条课程记录（更新 {updated} 条）"
            )

    # 图片生成由系统差遣（--only-image 子命令，复用爬虫 matplotlib 环境）。
    # 爬虫主流程不再自动生成图片（v6.14 爬虫越权收回阶段2），落库+周次同步后触发。
    # 图片失败不影响已完成的落库与进程标记。
    try:
        from app.services.spider_runner import run_spider_process as _run_img

        _img_result = _run_img(
            ["--only-image", "--semester-id", str(eams_id)], timeout=300
        )
        if _img_result.returncode == 0:
            logger.info(f"[爬取任务] 学期 {eams_id} 课程表图片已生成（系统差遣 --only-image）")
        else:
            logger.warning(
                f"[爬取任务] 学期 {eams_id} 图片生成失败（--only-image, code={_img_result.returncode}）"
            )
    except Exception as _ie:
        logger.warning(f"[爬取任务] 学期 {eams_id} 图片生成异常（--only-image）: {_ie}")
    ...
    if created == 0 and updated == 0:
        ...
        logger.info(
            f"[爬取任务] 学期 {eams_id} 重爬刷新完成（新增 0 条 / 更新 {updated} 条），属正常"
        )
    else:
...
def _crawl_all_semesters(timeout: int = 900):
    ...
    for idx, (eid, db_id) in enumerate(pairs, 1):
        ...
        try:
            created, updated = _crawl_one_semester(
                eid, db_id, week=None, timeout=timeout, scope_label="全部学期"
            )
            if created > 0:
...
def _run_scheduled_crawl(task_id: int):
    ...
    try:
        ...
        # 注：各学期的明细进程（课程全量爬取·学期X / ·全部学期）现已由系统侧
        # _crawl_one_semester 在落库后创建并标记（v6.14 爬虫越权收回），本伞进程
        # 代表整次爬取任务，二者互补，互不冲突。
        process_id = None
        try:
...
def reap_stale_crawl_tasks(session=None) -> int:
    ...
    try:
        ...
        stale = (
            session.query(ScheduledCrawlTask)
            .filter(ScheduledCrawlTask.status.in_([TaskStatus.RUNNING, TaskStatus.PENDING]))
            .all()
        )
        for t in stale:
            # 运行/等待起点：running 用 started_at，pending 用 created_at
            ref_time = (
                t.started_at if (t.status == TaskStatus.RUNNING and t.started_at) else t.created_at
            )
...
def dispatch_scheduled_crawls():
    ...
    try:
        ...
        candidates = (
            session.query(ScheduledCrawlTask)
            .filter(ScheduledCrawlTask.status == TaskStatus.PENDING)
            .all()
        )
        ...
        for t in due:
            logger.info(
                f"[爬取任务] 调度器拾取任务 id={t.id} (scope={t.scope}, schedule_type={t.schedule_type})"
            )
            import threading
