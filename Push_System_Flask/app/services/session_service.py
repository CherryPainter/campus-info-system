...
class SessionService:
    ...
    def create_session(
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"[Session] 创建Session成功: user_id={user_id}, session_id={session_id}"
                )
                return session_obj
            finally:
                db_session.close()
    ...
    @staticmethod
    def validate_session(session_id: str, check_ip: bool = False) -> User | None:
        """
        验证Session是否有效
        ...
        try:
            ...
            try:
                session_obj = (
                    db_session.query(ServerSession)
                    .filter(
                        and_(
                            ServerSession.session_id == session_id,
                            ServerSession.is_active.is_(True),
                        )
                    )
                    .first()
                )
                ...
                if check_ip and request:
                    ...
                    if session_obj.ip_address and session_obj.ip_address != client_ip:
                        logger.warning(
                            f"[Session] Session IP不匹配: {session_id}, 期望={session_obj.ip_address}, 实际={client_ip}"
                        )
                        return None
    ...
    def delete_session(session_id: str, reason: str = None, by_ip: str = None) -> bool:
        ...
        try:
            ...
            try:
                session_obj = (
                    db_session.query(ServerSession)
                    .filter(ServerSession.session_id == session_id)
                    .first()
                )
    ...
    def delete_all_user_sessions(
        ...
        try:
            ...
            try:
                ...
                db_session.commit()
                logger.info(
                    f"[Session] 删除用户所有Session: user_id={user_id}, count={count}, reason={reason}"
                )
                return count
            finally:
                db_session.close()
        ...
        try:
            ...
            try:
                ...
                active = (
                    ...
                    .order_by(ServerSession.updated_at.desc())
                    .all()
                )
                ...
                sessions = (
                    ...
                    .filter(
                        and_(
                            ...
                            ServerSession.is_active.is_(True),
                            or_(
                                ServerSession.expires_at is None,
                                ServerSession.expires_at > datetime.now(),
                            ),
                        )
                    )
        ...
        try:
            ...
            try:
                rows = (
                    ...
                    .join(User, ServerSession.user_id == User.id)
                    .filter(
                        and_(
                            ServerSession.is_active.is_(True),
                            or_(
