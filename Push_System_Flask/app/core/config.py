...
class Config:
    ...
    def reload(cls):
        ...
        cls.APP_VERSION = os.getenv("APP_VERSION", "6.15.0")
        ...
        _origins_raw = (
            ...
            or f"http://localhost:{cls.PORT},http://localhost:5173,http://localhost:5174"
        )
        ...
        cls.ENABLE_FOREIGN_IP_BLOCK = os.getenv("ENABLE_FOREIGN_IP_BLOCK", "true").lower() in (
            "1",
            "true",
            "yes",
            "on",
        )
        cls.REGION_BLOCK_EXCEPTIONS = [
            x.strip() for x in os.getenv("REGION_BLOCK_EXCEPTIONS", "").split(",") if x.strip()
        ]
        ...
        # 课程图片配置
        cls.COURSE_ENABLE_BACKGROUND = (
            os.getenv("COURSE_ENABLE_BACKGROUND", "true").lower() == "true"
        )

        # 课程推送开关（前端可修改）
        cls.COURSE_PUSH_ENABLED = os.getenv("COURSE_PUSH_ENABLED", "true").lower() == "true"
        cls.COURSE_DEFAULT_PUSH_ENABLED = (
            os.getenv("COURSE_DEFAULT_PUSH_ENABLED", "true").lower() == "true"
        )
        ...
        # 低电量告警
        cls.ELECTRICITY_LOW_POWER_THRESHOLD = float(
            os.getenv("ELECTRICITY_LOW_POWER_THRESHOLD", "10.0")
        )
        ...
        cls.ELECTRICITY_SCHEDULE_MONTHLY_DAY = int(
            os.getenv("ELECTRICITY_SCHEDULE_MONTHLY_DAY", "1")
        )
        ...
        cls.QWEATHER_PRIVATE_KEY_PATH = os.getenv(
