...
export default function Processes() {
  ...
  const [crawlPagination, setCrawlPagination] = useState({
    total: 0,
    page: 1,
    per_page: 20,
    pages: 0,
      ...
      <Alert
        message={
          <Space>
            ...
            <span>{statusMap[p.status]?.text}</span>
          </Space>
        }
        type={
          p.status === "completed"
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <Timeline
          items={[
            ...
            ...(p.status === "completed"
              ? [{ color: "green", children: `执行完成 ${formatDateTime(p.completed_at)}` }]
              : []),
            ...(p.status === "failed"
              ? [{ color: "red", children: `执行失败 ${formatDateTime(p.completed_at)}` }]
              : []),
            ...(p.status === "cancelled"
              ? [{ color: "gray", children: `已取消 ${formatDateTime(p.completed_at)}` }]
              : []),
            ...(p.status === "skipped"
              ? [
                  {
                    color: "orange",
                    children: `${p.message || "已静默"} ${formatDateTime(p.completed_at || p.started_at)}`,
                  },
                ]
              : []),
          ]}
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div
            style={{
              padding: 12,
              background:
                p.status === "failed" ? "#fff2f0" : p.status === "skipped" ? "#fffbe6" : "#f6ffed",
              borderRadius: 4,
  ...
  const renderCrawlDetail = (t: CrawlTask) => (
    <Descriptions bordered column={1} size="small">
      ...
      <Descriptions.Item label="执行方式">
        {t.schedule_type === "immediate"
          ? "立即执行"
          : `预约 ${t.scheduled_at ? formatDateTime(t.scheduled_at) : "-"}`}
      </Descriptions.Item>
      <Descriptions.Item label="状态">
        {crawlStatusMap[t.status]?.text || t.status}
      </Descriptions.Item>
  ...
  const scheduledColumns = [
    { title: "名称", dataIndex: "name", key: "name", width: 200 },
    {
      ...
      width: 80,
      render: (type: string) => (
        <Tag color={type === "cron" ? "blue" : "green"}>{type === "cron" ? "定时" : "间隔"}</Tag>
      ),
    },
    {
      title: "执行频率",
          ...
          countdown === "即将执行"
            ...
            : countdown === "执行中"
              ...
              : countdown === "等待更新"
                ? "blue"
                : "default";
        return countdown ? <Tag color={tagColor}>{countdown}</Tag> : "-";
      },
    },
  ];
  ...
  return (
    <div>
            ...
            children: (
              ...
              >
                ...
                {scheduledJobs.length > 0 && (
                  <div style={{ marginBottom: 16 }}>
                    <div
                      style={{ fontWeight: 600, marginBottom: 8, color: "#1890ff", fontSize: 14 }}
                    >
                ...
                {dynamicRules.length > 0 && (
                  <div style={{ marginBottom: 16 }}>
                    <div
                      style={{ fontWeight: 600, marginBottom: 8, color: "#52c41a", fontSize: 14 }}
                    >
                    ...
                    <ResponsiveTable
                      ...
                      size="small"
                      scroll={{ x: 600 }}
                      mobileCollapseHeader={(r: DynamicRule) =>
                        collapseHeader(
                          r.name,
                          <Tag
                  ...
                  <Space>
                    <Select
                      ...
                      onChange={(value) => {
                        ...
                        fetchProcesses(1, value || "", filterTaskType);
                      }}
                    >
                    ...
                    <Select
                      ...
                      onChange={(value) => {
                        ...
                        fetchProcesses(1, filterStatus, value || "");
                      }}
                    >
                ...
                <ResponsiveTable
                  ...
                  mobileCollapseHeader={(r: TaskProcess) =>
                    collapseHeader(
                      r.name,
                      <Badge
                        status={statusMap[r.status]?.color as any}
              ...
              <Card title="爬取预约任务">
                ...
                >
                  <Space>
                    ...
                    <Button
                      icon={<ReloadOutlined />}
                      onClick={() => fetchCrawlTasks(crawlPagination.page)}
                      size="small"
                    >
                      刷新
                    </Button>
                  ...
                  columns={[
                    {
                      ...
                      render: (t: string, r: CrawlTask) => (
                        <a
                          onClick={() => {
                            setSelectedCrawlTask(r);
                            setCrawlDetailVisible(true);
                          }}
                        >
                          {t}
                        </a>
                      ...
                      render: (_: any, r: CrawlTask) => (
                        <Space size={4}>
                          ...
                          {(r.status === "pending" || r.status === "running") && (
                            <Popconfirm
                              title="确定取消该任务吗？"
                              onConfirm={() => handleCrawlCancel(r.id)}
                            >
                          ...
                          <Popconfirm
                            title="确定删除吗？"
                            onConfirm={() => handleCrawlDelete(r.id)}
                          >
                            <Button type="link" size="small" danger icon={<DeleteOutlined />}>
                              删除
                            </Button>
      ...
      <Modal
        ...
        footer={[
          <Button key="close" onClick={() => setCrawlDetailVisible(false)}>
            关闭
          </Button>,
        ]}
        width={640}
