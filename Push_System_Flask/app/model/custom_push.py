...
class CustomPush(Base):
    ...
    msg_type = Column(String(20), default='text', comment='消息类型: text文本, image图片, template模板')
    ...
    push_type = Column(String(20), default='immediate', comment='推送类型: immediate立即, scheduled定时, recurring周期')
    ...
    status = Column(String(20), default='pending', comment='状态: pending待发送, sent已发送, failed失败, cancelled已取消')
