...
验证 `app/tasks/scheduler.run_spider`（每日课表爬虫，7:00/13:00 各一次）与假期模式、
...
from app.services import spider_runner as spider_runner_mod
from app.api import process_routes as process_routes_mod
...
def reset_state():
    ...
    scheduler_mod._spider_running = False
    yield
    scheduler_mod._spider_running = False
...
class TestRunSpiderHolidayLinkage:
    ...
    def test_proceeds_when_in_teaching_week_and_no_holiday(self, monkeypatch, reset_state, no_real_process):
        """在教学周内且未开假期模式：run_spider 应真正调用爬虫子进程。"""
        monkeypatch.setattr(hs_mod.holiday_service, 'is_active', lambda: (False, None))
        monkeypatch.setattr(scheduler_mod, '_is_in_teaching_week', lambda: True)
        called = []
        monkeypatch.setattr(scheduler_mod, 'run_spider_process',
    ...
    def test_teaching_week_check_before_crawl_avoids_false_failure(self, monkeypatch, reset_state):
        ...
        monkeypatch.setattr(scheduler_mod, '_is_in_teaching_week', lambda: False)
        spider_called = []
        monkeypatch.setattr(scheduler_mod, 'run_spider', lambda *a, **k: spider_called.append(1) or True)
