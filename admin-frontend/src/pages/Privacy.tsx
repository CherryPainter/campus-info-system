  ...
  return (
    <div style={{ maxWidth: 900, margin: '40px auto', padding: '0 24px' }}>
      <Card bordered={false} style={{ borderRadius: 12, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
        ...
        <Paragraph>
          本系统<Text strong>不收集、不存储用户的个人身份信息</Text>（如真实姓名、学号、身份证号、手机号等）。
          系统仅存储保障基本功能运行所需的最少数据：
        </Paragraph>
        <Paragraph>
          <ul>
        ...
        <Paragraph>
          <ul>
            <li><Text strong>身份认证</Text>：验证用户登录合法性，保障系统安全；</li>
            <li><Text strong>信息推送</Text>：天气通知、电量告警、课程提醒等核心推送功能；</li>
            <li><Text strong>数据展示</Text>：在系统管理后台中展示课程表、电量趋势、天气信息等聚合数据。</li>
          </ul>
        </Paragraph>
        <Paragraph>
        ...
        <Paragraph>
          3.1 <Text strong>存储位置</Text>：所有数据存储于本系统所在的服务器上，不涉及云端第三方存储。
        </Paragraph>
        <Paragraph>
        ...
        <Paragraph>
          本系统使用必要的 httpOnly Cookie（access_token、refresh_token 与 session_id）来维持用户会话状态，
          不会追踪您在其他网站的活动。其中会话整体有效期取决于登录时是否勾选「记住我」。
          禁用 Cookie 可能导致无法正常登录本系统。详细信息请查阅我们的 <Link href="/cookies">Cookie 政策</Link>。
        </Paragraph>
        ...
        <Paragraph>
          <ul>
            <li><Text strong>和风天气 API</Text>：用于获取天气数据，查询请求发送至和风天气服务器；</li>
            <li><Text strong>企业微信 Webhook</Text>：用于推送消息通知至指定群聊；</li>
            <li><Text strong>教务系统</Text>：爬取课程数据时需使用教务系统账户凭证，该凭证仅用于数据获取，不会被传播或用于其他目的。</li>
          </ul>
        </Paragraph>
        ...
        <Paragraph>
          如您对本隐私政策有任何疑问，请通过<Link href="/contact">联系我们</Link>页面中提供的方式与我们取得联系。
        </Paragraph>
      </Card>
    </div>
  );
