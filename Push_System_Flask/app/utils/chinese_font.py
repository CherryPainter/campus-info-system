...
_FONT_KEYWORDS = [
    ...
    "nsimsun",  # Windows 宋体
    "wqy-microhei",
    "wqy-zenhei",  # Linux 文泉驿字体
    "noto sans cjk sc",
    "noto sans cjk tc",  # Noto CJK 简体/繁体
    "noto sans cjk",
    "notocjk",
    "noto",  # Noto CJK 通用
    "source han sans sc",
...
def _verify_dejavu():
    ...
    try:
        path = fm.findfont(fm.FontProperties(family="DejaVu Sans"), fallback_to_default=False)
...
def setup_chinese_font(force_rebuild_cache=False):
    ...
    try:
        ...
        if path:
            logger.warning("找到中文字体但无法安全使用，已回退 DejaVu Sans（中文可能显示为方块）")
        else:
            logger.warning("未找到可加载的中文字体，使用 DejaVu Sans（中文可能显示为方块）")
    except Exception as e:
