#!/usr/bin/env python3
# ruff: noqa: F401, F811  # 本文件集中导入全部模型以注册到 SQLAlchemy Base.metadata，F401/F811 为预期行为
"""Flask 应用工厂"""
...
from app.core.bootstrap import bootstrap_command, run_bootstrap
...
# 启动期引导逻辑（建库/建管理员/指纹迁移）已收敛到 app.core.bootstrap，
# 由 create_app 在 AUTO_MIGRATE_ON_START 开启时调用，亦可 flask bootstrap 手动执行。
...
def create_app(config_class=None):
    ...
    def bad_request(error):
        ...
        return jsonify({"status": "error", "message": "请求参数错误，请检查输入"}), 400

    @app.errorhandler(401)
    def unauthorized(error):
        """401 Unauthorized - 返回通用认证失败信息"""
        logger.warning(f"401 Unauthorized: {error}")
        return jsonify({"status": "error", "message": "认证失败，请重新登录"}), 401

    @app.errorhandler(403)
    def forbidden(error):
        """403 Forbidden - 返回通用权限不足信息"""
        logger.warning(f"403 Forbidden: {error}")
        return jsonify({"status": "error", "message": "权限不足，无法访问"}), 403
    ...
    def method_not_allowed(error):
        ...
        return jsonify({"status": "error", "message": "HTTP 方法不允许"}), 405

    @app.errorhandler(413)
    def request_too_large(error):
        """413 Request Too Large - 返回通用请求过大信息"""
        logger.warning(f"413 Request Too Large: {error}")
        return jsonify({"status": "error", "message": "请求大小超过限制"}), 413
    ...
    # 启动期引导（建库 / 建默认管理员 / 指纹自动迁移）——已收敛到 app.core.bootstrap。
    # AUTO_MIGRATE_ON_START 默认开启，行为与原版一致；排障时可置 False 关闭，
    # 改为手动执行 `flask bootstrap`。
    ...
    # 注册 flask bootstrap 命令（手动触发启动期引导）
    app.cli.add_command(bootstrap_command)
