...
def no_alert():
    ...
    with mock.patch.object(svc.IPBlacklistService, "_send_block_alert", lambda *a, **k: None):
        yield
...
def test_block_and_is_blocked_permanent(session):
    rec = svc.IPBlacklistService.block_ip(session, "2.2.2.2", reason="manual", source="manual")
...
def test_record_event_auto_block(session):
    ...
    blocked_events = (
        session.query(IPSecurityEvent).filter_by(ip_address="8.8.8.8", is_blocked=True).count()
    )
...
def test_ignore_event(session):
    ev = svc.IPBlacklistService.record_event(session, "9.9.9.9", "suspicious_path", path="/y")
...
def test_ban_event_ip(session):
    ev = svc.IPBlacklistService.record_event(session, "10.10.10.10", "sql_injection", path="/z")
    assert svc.IPBlacklistService.is_ip_blocked(session, "10.10.10.10") is False
    ok, msg = svc.IPBlacklistService.ban_event_ip(session, ev.id, reason="test", duration_hours=2)
    assert ok is True
...
def test_cleanup_expired(session):
    expired = IPBlacklist(
        ip_address="11.11.11.11", is_active=True, expires_at=datetime.now() - timedelta(hours=1)
    )
    permanent = IPBlacklist(ip_address="12.12.12.12", is_active=True, expires_at=None)
