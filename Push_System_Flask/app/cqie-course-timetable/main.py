...
from parser_utils import (
    _count_distinct_weeks,
    _split_js_args,
    _unquote,
    _weeks_bitmap_to_str,
)
...
try:
    import xlsx_import  # noqa: F401  # 仅探测模块是否存在（可用性开关由 XLSX_IMPORT_AVAILABLE 控制）
...
class CourseTableTool:
            ...
            if resp.ok:
                ...
                if OCR_AVAILABLE:
                    ...
                    if captcha and len(captcha) >= 4 and captcha.isdigit():
                        return captcha
                    self.logger.warning(
                        f"验证码不符合要求（非纯数字或长度不足）: {captcha if captcha else '空'}"
                    )
            else:
    ...
    async def _login(self, page, max_retries=3):
        ...
        for retry in range(max_retries):
            ...
            if not captcha or len(captcha) < 4 or not captcha.isdigit():
                self.logger.warning(
                    f"[重试 {retry + 1}/{max_retries}] 验证码识别失败（非纯数字或长度不足），正在重试..."
                )
                continue
            ...
            if error_text:
                if "验证码错误" in error_text:
                    ...
                    self.logger.error(
                        "[失败] 账号或密码错误！请检查config.py中的配置！程序将退出！"
                    )
                    return False
                else:
            ...
            for i in range(30):
                ...
                if i % 5 == 0 and i > 0:
                    ...
                    if error_text:
                        if "验证码错误" in error_text:
                            self.logger.warning(
                                f"[重试 {retry + 1}/{max_retries}] 验证码错误，正在重试..."
                            )
    ...
    def _extract_teacher_mapping_from_html(self, html):
        ...
        for match in matches:
            match[0].strip()  # actTeacherName.join(',')
                ...
                if teacher_mapping and cell_text:
                    ...
                    if code_m:
                        ...
                        if teacher_name and teacher_name not in cell_text:
                            ...
                            cell_text = (
                                cell_text[:insert_at] + f"({teacher_name})" + cell_text[insert_at:]
                            )
    ...
    def _parse_activities(self, html):
        ...
        for i, m in enumerate(ta_iter):
            ...
            name_raw = _unquote(args[3])
            name_raw = re.sub(r"<[^>]+>", "", name_raw)
            course_name = re.sub(r"\([^)]*\)\s*$", "", name_raw).strip()

            # 教室（已含"(校本部)"）与整学期周次位图
            room = _unquote(args[5])
            bitmap = _unquote(args[6])
            weeks_str = _weeks_bitmap_to_str(bitmap)
            ...
            slots = [
                (int(d), int(p))
                for d, p in re.findall(r"index\s*=\s*(\d+)\s*\*\s*unitCount\s*\+\s*(\d+)", seg)
            ]
    ...
    def _build_grid_from_activities(self, activities, unit_count=12):
        ...
        for act in activities:
            ...
            cell = (
                f"{act['course_name']}({act['code']}){teacher_part}"
    ...
    async def _download_xlsx_export(self, page, save_dir):
        ...
        if form_el:
            btn = await form_el.query_selector(
            ...
            if href and not href.lower().startswith("javascript"):
                ...
                async with page.context.expect_download(timeout=45000) as dl_info:
                    if tag == "FORM":
                        ...
                        submit_btn = await chosen.query_selector(
    ...
    def _build_grid_from_xlsx(self, xlsx_courses, unit_count=12):
        ...
        for c in xlsx_courses:
            ...
            cell = (
                f"{c['course_name']}({c['course_code']}){teacher_part}"
        ...
        try:
            ...
            for wsel in ('select[name="week.id"]', 'select[name*="week"]', "#weekBar"):
                ...
                if wsel_el:
                    try:
                        wvals = [
                            int((await wo.get_attribute("value") or "").strip())
    ...
    async def _switch_semester(self, page, semester_id):
        ...
        try:
            ...
            if "TaskActivity" not in html:
                self.logger.warning(
                    "切换学期后 60s 内未探测到 TaskActivity，交由主流程继续渲染（选全部后兜底）..."
                )
            else:
    ...
    async def _get_all_weeks_auto(self, max_weeks=20):
        ...
        try:
            ...
            self.logger.info(
                f"从 TaskActivity 解析到 {len(acts)} 门课程（unit_count={unit_count}）"
            )
            ...
            try:
                covered = _count_distinct_weeks(acts)
        ...
        try:
            ...
            while True:
                ...
                if course_data:
                    ...
                    self.logger.info(
                        f"第 {current_week} 周数据保存成功，包含 {len(course_data.get('rows', []))} 行"
                    )
                else:
    ...
    async def _select_week_and_wait(self, page, week):
        ...
        try:
            ...
            if warning:
                ...
                self.logger.warning(
                    f"第 {week} 周触发'过快点击'提示（已被自动关闭，不影响渲染），继续..."
                )
    ...
    def _merge_all_weeks_data(self, all_course_data):
        ...
        # 使用第一周的数据作为基础
        merged = {"headers": all_course_data[0].get("headers", []), "rows": [], "all_weeks": []}
        ...
        for week_data in all_course_data:
            ...
            merged["all_weeks"].append({"week_number": week_number, "row_count": len(rows)})
...
def main():
    ...
    parser.add_argument(
        "--all-weeks", dest="all_weeks", action="store_true", help="爬取所有周次（1-20周）"
    )
    parser.add_argument(
        "--interactive",
        dest="interactive",
    ...
    parser.add_argument(
        "--max-weeks", dest="max_weeks", type=int, default=20, help="最大周次（默认20）"
    )
    parser.add_argument(
        "--semester-id",
        dest="semester_id",
    ...
    parser.add_argument(
        ...
        dest="only_image",
        action="store_true",
        help="仅图片模式：跳过爬取与处理，根据已产出的 CSV 重新生成课程表图片（由系统差遣）",
    )
    args, _ = parser.parse_known_args()

    # 仅图片模式：跳过耗时的爬取/处理，直接根据已产出的 CSV 生成图片。
    # 由系统侧在落库成功后差遣调用（python main.py --only-image [--semester-id X]）。
    if args.only_image:
        return _run_image_only(args, logger)
    ...
    try:
        ...
        if args.all_weeks:
            if args.interactive:
                ...
                logger.info(
                    f"将使用交互模式爬取所有周次（1-{args.max_weeks}周），请手动切换周次并按回车保存"
                )
            else:
            ...
            if args.all_weeks:
                ...
                async def run_with_timeout():
                    return await tool.get_all_weeks_course_table(
                        max_weeks=args.max_weeks, interactive=args.interactive
                    )
            else:
        ...
        # 图片生成已拆出为 --only-image 子命令，由系统侧落库成功后差遣触发，
        # 爬虫主流程不再自动生成图片（v6.14 爬虫越权收回阶段2，系统夺回控制权）。
        logger.info("课程数据已产出（JSON/CSV），图片由系统侧 --only-image 差遣生成")
...
def _run_image_only(args, logger):
    """仅图片模式：跳过爬取与处理，根据已产出的 CSV 重新生成课程表图片。

    由系统侧在落库成功后差遣调用（python main.py --only-image [--semester-id X]）。
    读取已产出的处理后 CSV（考虑学期隔离目录），调用 CsvToImage 生成图片。
    """
    try:
        script_dir = os.path.dirname(os.path.abspath(__file__))
        processed_data_dir = os.path.join(script_dir, CONFIG["processing"]["processed_data_dir"])
        ...
        csv_files = [
            f for f in os.listdir(processed_data_dir) if f.endswith(".csv") and "_week" in f
        ]
        if not csv_files:
            logger.warning("未找到处理后的CSV文件，跳过图片生成（--only-image）")
            return 0

        csv_files.sort(
            key=lambda x: os.path.getmtime(os.path.join(processed_data_dir, x)), reverse=True
        )
        ...
        if IMAGE_GEN_AVAILABLE:
            ...
            if image_path:
                logger.info(f"[仅图片] 课程表图片生成成功: {image_path}")
                return 0
            logger.error("[仅图片] 生成课程表图片失败！")
            return 1
        logger.info("[仅图片] 图片生成功能不可用，跳过")
        return 0
    except Exception as e:
        logger.error(f"[仅图片] 生成图片出错: {e}", exc_info=True)
        return 1
