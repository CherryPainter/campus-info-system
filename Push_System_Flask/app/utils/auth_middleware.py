...
def jwt_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            return jsonify({"status": "error", "message": "认证令牌已过期，请刷新 token"}), 401
        except (jwt.InvalidTokenError, ValueError) as e:
            logger.debug(f"JWT 认证: token 无效 - {e}")
            return jsonify({"status": "error", "message": "认证令牌无效"}), 401
        except RuntimeError:
            return jsonify({"status": "error", "message": "服务器配置错误"}), 500

        # 3. 检查 token 类型（只允许 access token 用于 API 认证）
        if payload.get("type") != "access":
            logger.warning(f'JWT 认证失败: token 类型错误 (type={payload.get("type")})')
            return jsonify({"status": "error", "message": "令牌类型错误，请使用 access token"}), 401
            ...
            try:
                ...
                if detail["status"] != "active":
                    ...
                    if reason == "new_login":
                        ...
                        msg = "登录会话已失效，请重新登录"
                    logger.warning(
                        f"JWT 认证: 关联 Session 已失效 (session_id={sid}, reason={reason})"
                    )
                    return api_error(
                        message=msg, http_status=401, revoke_reason=reason, revoke_ip=ip
                    )
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if role != "admin":
            logger.warning(f'管理员权限验证失败: user={user.get("username")}, role={role}')
            return jsonify({"status": "error", "message": "权限不足，需要管理员权限"}), 403
