...
def _check_login_rate_limit():
    ...
    with _login_lock:
        ...
        if len(_login_attempts[client_ip]) >= LOGIN_RATE_LIMIT:
            return api_error(
                message=f"登录尝试过于频繁，请在 {LOGIN_RATE_WINDOW} 秒后重试", http_status=429
            )
...
def _login_failure_response(client_ip, username, kind, user_id, user_agent):
    ...
    try:
        ...
        if action == "rate_limit":
            ...
            _bs.commit()
            _record_login_log(user_id or 0, username, client_ip, user_agent, "rate_limited", label)
            return api_error(
...
def login():
    ...
    if _ip_is_blocked(client_ip):
        if not (username and password):
            _record_login_log(
                0, username or "(空)", client_ip, user_agent, "blocked", "IP 已被封禁"
            )
            return api_error(message="拒绝访问", http_status=403)
        ...
        if not _ok:
            ...
            _record_login_log(
                _u.id if _u else 0, username, client_ip, user_agent, "blocked", "IP 已被封禁"
            )
            return api_error(message="拒绝访问", http_status=403)
    ...
    if user.role == "admin" and not mfa_enabled and current_app.config.get("FORCE_ADMIN_MFA", True):
        ...
        if _any_mfa_enabled:
            ...
            _record_login_log(
                user.id, username, client_ip, user_agent, "failed", "管理员未启用 MFA"
            )
            return api_error(
    ...
    if sid:
        try:
            ...
            if detail["status"] != "active":
                ...
                if reason == "new_login":
                    msg = (
                        f"登录会话已在其他设备（IP：{ip}）登录，当前会话已失效，请重新登录"
                        if ip
                        else "登录会话已在其他设备登录，当前会话已失效，请重新登录"
                    )
                elif reason == "expired":
                    msg = "登录会话已过期，请重新登录"
                    ...
                    msg = "登录会话已失效，请重新登录"
                logger.warning(
                    f"Token 刷新失败: 关联 Session 已失效 (session_id={sid}, reason={reason})"
                )
...
def mfa_setup():
    ...
    return api_success(
        message="请使用 Google Authenticator 或类似应用扫描二维码",
        data={
            "secret": mfa_config["secret"],
            "provisioning_uri": mfa_config["provisioning_uri"],
