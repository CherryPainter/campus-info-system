...
from app.model.module_config import DEFAULT_CONFIGS, ModuleConfig, init_default_configs
    ...
    try:
        ...
        for config in configs:
            ...
            if module not in grouped:
                grouped[module] = {"name": MODULE_NAMES.get(module, module), "configs": []}
...
def update_config(module: str, key: str):
    ...
    try:
        ...
        try:
            ...
            # 重新加载配置
            Config.reload()
            logger.info("[配置管理] Config 类已重新加载")
            ...
            if app:
                ...
                if module in ("course", "electricity", "weather") and key in (
                    "spider_enabled",
                    "spider_interval_hours",
                    "spider_schedule_mode",
                ...
                if module == "course" and key in (
                    ...
                    try:
                        ...
                        logger.info(
                            f"[配置管理] 规则引擎触发缓存已清空 ({module}.{key} = {new_value})"
                        )
