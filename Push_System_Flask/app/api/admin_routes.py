...
def _get_modules_status():
    ...
    try:
        ...
        modules["electricity"] = {
            ...
            "config": {
                "low_power_threshold": current_app.config.get(
...
def _time_range_label(time_range, date_range):
    ...
    if date_range is None:
        return "全部时间"
    try:
        range_label = date_range[0].strftime("%m/%d") + " - " + date_range[1].strftime("%m/%d")
        map_ = {
            "this_month": "本月",
            "last_month": "上月",
            "this_week": "本周",
        ...
        return map_.get(time_range, range_label)
    except Exception:
        return "自定义"
...
def _get_tasks_info(date_range=None):
    ...
    try:
        ...
        try:
            ...
            # 各状态数量
            status_counts = {}
            for row in _with_period(
                session.query(TaskProcess.status, func.count(TaskProcess.id)).group_by(
                    TaskProcess.status
                )
            ).all():
            ...
            period_completed = (
                _with_period(
                    session.query(func.count(TaskProcess.id)).filter(
            ...
            period_failed = (
                _with_period(
                    session.query(func.count(TaskProcess.id)).filter(TaskProcess.status == "failed")
                ).scalar()
                or 0
            )
            ...
            # 今日 / 本周 / 本月 保留（前端卡片备用）
            today_total = (
                session.query(func.count(TaskProcess.id))
                .filter(TaskProcess.started_at >= today_start)
                .scalar()
                or 0
            )
            today_completed = (
                session.query(func.count(TaskProcess.id))
                .filter(TaskProcess.started_at >= today_start, TaskProcess.status == "completed")
                .scalar()
                or 0
            )
            today_failed = (
                session.query(func.count(TaskProcess.id))
                .filter(TaskProcess.started_at >= today_start, TaskProcess.status == "failed")
                .scalar()
                or 0
            )
            week_total = (
                session.query(func.count(TaskProcess.id))
                .filter(TaskProcess.started_at >= week_start)
                .scalar()
                or 0
            )
            month_total = (
                session.query(func.count(TaskProcess.id))
                .filter(TaskProcess.started_at >= month_start)
                .scalar()
                or 0
            )
            ...
            type_counts = {}
            for row in _with_period(
                session.query(TaskProcess.task_type, func.count(TaskProcess.id)).group_by(
            ...
            if dt_filter is not None:
                ...
                trend_rows = (
                    ...
                    .filter(dt_filter)
                    .group_by("day", TaskProcess.task_type)
                    .order_by("day")
                    .all()
                )
                ...
                # 按任务类型聚合成 series
                type_set = sorted({t for (_, t, _) in trend_rows})
    ...
    try:
        ...
        for key, default in defaults.items():
            cfg = (
                ...
                .filter(ModuleConfig.module == "weather", ModuleConfig.key == key)
                .first()
            )
...
def trigger_weather_task():
    ...
    if task_type not in task_map:
        return api_error(
            ...
            supported_types=list(task_map.keys()),
            http_status=400,
        )
...
def get_electricity_config():
    ...
    return api_success(
        data={
            ...
            "low_power_interval_hours": current_app.config.get(
                "ELECTRICITY_LOW_POWER_INTERVAL_HOURS", 4.0
            ),
            "daily_push_time": current_app.config.get("ELECTRICITY_SCHEDULE_DAILY", "00:30"),
            "weekly_push_time": current_app.config.get("ELECTRICITY_SCHEDULE_WEEKLY", "00:30"),
            "weekly_push_day": current_app.config.get("ELECTRICITY_SCHEDULE_WEEKLY_DAY", "mon"),
            "monthly_push_time": current_app.config.get("ELECTRICITY_SCHEDULE_MONTHLY", "00:30"),
            "monthly_push_day": current_app.config.get("ELECTRICITY_SCHEDULE_MONTHLY_DAY", 1),
            "cookie_check_time": current_app.config.get("ELECTRICITY_COOKIE_CHECK_TIME", "20:00"),
        }
    )
...
def trigger_course_task():
    ...
    try:
        ...
        # 假期 / 非教学周 双条件拦截（与手动触发爬虫同源，run_spider 内部亦同）：
        # 假期模式静默或非教学周时不推送课表，避免假期期间仍向用户推送当日/本周课程。
        holiday_active, holiday_period = holiday_service.is_active()
        in_teaching_week = _is_in_teaching_week()
        if holiday_active or not in_teaching_week:
            if holiday_active:
                reason = (
                    f"假期模式静默（{holiday_period.name}）" if holiday_period else "假期模式静默"
                )
                msg = f"{reason}，已跳过{label}"
            else:
                msg = f"当前不在教学周内（假期/长假），已跳过{label}"
            logger.info(f"[管理后台] 手动触发课程任务被跳过: {msg}")
            ...
            if rule:
                ...
                if task:
                    ...
                    complete_task_process(
                        pid, "completed", f"{label} 成功，共推送 {len(today_schedules)} 节课"
                    )
                else:
...
def trigger_spider():
    ...
    try:
        from app.services.holiday_service import holiday_service
        from app.tasks.scheduler import _is_in_teaching_week, get_spider_status, run_spider
        ...
        thread = threading.Thread(
            target=run_spider, kwargs={"trigger_source": "manual"}, daemon=True
        )
        thread.start()
...
def spider_status():
    ...
    try:
        ...
        try:
            ...
            elec_running = (
                session.query(_TP)
                .filter(_TP.name.like("电量全量爬取%"), _TP.status == "running")
                .first()
                is not None
            )
        finally:
...
def get_system_config():
    ...
    safe_config_keys = [
        ...
        "ELECTRICITY_LOW_POWER_THRESHOLD",
        "ELECTRICITY_LOW_POWER_INTERVAL_HOURS",
        "ELECTRICITY_SCHEDULE_DAILY",
        "ELECTRICITY_SCHEDULE_WEEKLY",
        "ELECTRICITY_SCHEDULE_WEEKLY_DAY",
        "ELECTRICITY_SCHEDULE_MONTHLY",
        "ELECTRICITY_SCHEDULE_MONTHLY_DAY",
        "ELECTRICITY_COOKIE_CHECK_TIME",
