...
class WeComAdapter(BaseAdapter):
    ...
    def send_image(self, image_path):
        ...
        try:
            ...
            logger.info(
                f"send_image: 发送图片 {os.path.basename(image_path)} ({file_size / 1024:.1f}KB, base64长度: {len(b64)})"
            )

            result = self.send({"msgtype": "image", "image": {"base64": b64, "md5": md5}})
    ...
    def _upload_media(self, image_path):
        ...
        try:
            ...
            upload_url = (
                f"https://qyapi.weixin.qq.com/cgi-bin/webhook/upload_media?key={key}&type=image"
            )
            file_size = os.path.getsize(image_path)
            logger.info(
                f"_upload_media: 上传图片 {os.path.basename(image_path)} ({file_size / 1024:.1f}KB, {mime_type})"
            )
...
class MultiWeComAdapter(BaseAdapter):
    ...
    def __init__(self, config=None):
        ...
        if len(unique_urls) != len(webhook_urls):
            logger.warning(
                f"Webhook URL 去重: 从 {len(webhook_urls)} 个减少到 {len(unique_urls)} 个"
            )

        for i, url in enumerate(unique_urls):
            adapter = WeComAdapter({"webhook_url": url, "name": f"{self.name}_{i}"})
...
class AdapterService:
    ...
    def _load_webhooks_from_env(self):
        ...
        if webhook_urls:
            ...
            if len(unique_urls) != len(webhook_urls):
                logger.warning(
                    f"Webhook URL 去重 (env): 从 {len(webhook_urls)} 个减少到 {len(unique_urls)} 个"
                )
        ...
        if status_urls:
            ...
            if len(unique_urls) != len(status_urls):
                logger.warning(
                    f"状态 Webhook URL 去重 (env): 从 {len(status_urls)} 个减少到 {len(unique_urls)} 个"
                )
