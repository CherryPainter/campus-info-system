...
from course_processing.process_course_data import CourseProcessor, extract_teacher_map_from_html
...
with app.app_context():
    ...
    old_count = session.query(Course).filter(Course.is_deleted.is_(False)).count()
    if old_count > 0:
        session.query(Course).filter(Course.is_deleted.is_(False)).update(
            {
                "is_deleted": True,
    ...
    new_total = session.query(Course).filter(Course.is_deleted.is_(False)).count()
    print(
        f"[5] 入库完成: 新增 {created_count} 门 / 更新 {updated_count} 门, 当前有效 {new_total} 门"
    )

    # 验证教师填充
    with_t = (
        session.query(Course)
        .filter(Course.is_deleted.is_(False), Course.teacher != "", Course.teacher.isnot(None))
        .count()
    )
