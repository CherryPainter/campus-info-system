...
def revoke_session(session_id):
    ...
    db_session = get_db()
    try:
        target_session = db_session.query(ServerSession).filter(
        ...
        target_user = db_session.query(User).filter(
            User.id == target_session.user_id
        ).first()
        if not target_user:
        ...
        if operator_role == 'user':
            ...
            # 管理员需区分超管 / 普管
            operator = db_session.query(User).filter(
                User.id == int(operator_id)
            ).first()
...
def revoke_all_sessions():
    ...
    return api_success(message=f'已撤销 {count} 个其他设备Session', data={'count': count}, http_status=200)
