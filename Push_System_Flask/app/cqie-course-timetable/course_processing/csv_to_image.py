...
class CsvToImage:
    ...
    def generate_image(self, filename=None):
        ...
        try:
            ...
            if enable_background:
                # 使用绝对路径加载背景图片
                background_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'static', 'background.png'))
        ...
        week_number = csv_basename.split('_week')[1].split('.')[0] if '_week' in csv_basename else '1'
        ...
        # 设置表格标题（显式指定中文字体）
        plt.title(title, fontsize=image_config['title_font_size'], pad=10, fontweight='bold', fontproperties=self.chinese_font)
    ...
    def _generate_empty_placeholder(self, filename=None):
        ...
        ax.axis('off')
        plt.title(title, fontsize=image_config['title_font_size'], pad=10, fontweight='bold',
                  fontproperties=self.chinese_font)
        ax.text(0.5, 0.45, '暂无排课数据\n（本学期可能尚未排课）',
                ha='center', va='center', transform=ax.transAxes,
                fontsize=image_config['title_font_size'], fontproperties=self.chinese_font,
                color='#888888')
...
if __name__ == '__main__':
    ...
    # 默认处理最新的处理后CSV文件
    processed_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'output', 'course-data', 'processed'))
