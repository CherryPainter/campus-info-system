...
class DeliveryService:
    ...
    def _update_push_process(self, process_id, status, message=None, error=None, progress=None):
        ...
        if not process_id:
            return
        try:
            uts.complete_process(
                process_id, status=status, message=message, error=error, progress=progress
            )
                    ...
                    if adapter:
                        ...
                        if result.get("success"):
                            ...
                            self._update_push_process(
                                process_id, "completed", message="推送成功", progress=100
                            )
                        else:
                            task_service.update_status(
                                task["task_id"], "retrying", {"error": result.get("error")}
                            )
                            self._update_push_process(
                                process_id, "failed", message="推送失败", error=result.get("error")
                            )
                    else:
                        task_service.update_status(
                            task["task_id"], "failed", {"error": f"No adapter for {adapter_name}"}
                        )
                        self._update_push_process(
                            process_id,
                            "failed",
                            message="推送失败",
                            error=f"No adapter for {adapter_name}",
                        )
                        self._notify_delivery_failure(task, f"No adapter for {adapter_name}")
                else:
                    task_service.update_status(
                        task["task_id"], "failed", {"error": "Template not found"}
                    )
                    self._update_push_process(
                        process_id, "failed", message="推送失败", error="Template not found"
                    )
    ...
    def _send_image(self, task, process_id=None):
        ...
        if not adapter:
            ...
            self._update_push_process(
                process_id, "failed", message="推送失败", error="No adapter available"
            )
            self._notify_delivery_failure(task, "No adapter available")
            return
        if not hasattr(adapter, "send_image"):
            task_service.update_status(
                task["task_id"], "failed", {"error": "Adapter does not support image"}
            )
            self._update_push_process(
                process_id, "failed", message="推送失败", error="Adapter does not support image"
            )
        ...
        if result.get("success"):
            ...
            logger.error(
                f"图片任务 {task['task_id']} 发送失败: adapter={adapter_name}, error={error_detail}, result={result}"
            )
            ...
            if retry_count >= task.get("max_retries", 3):
                ...
                self._update_push_process(
                    process_id,
                    "running",
                    message=f'重试中 ({retry_count}/{task.get("max_retries", 3)})',
                )
    ...
    def _prepare_data(self, task):
        ...
        if task["task_type"] == "course_reminder":
            ...
            data = {
                ...
                "minutes_before_end": task.get("trigger_condition", {}).get(
                    "minutes_before_end", ""
                ),
            }
            # 下节课信息
            ...
            if isinstance(courses, list):
                ...
                lines = []
                for _i, c in enumerate(sorted_courses, 1):
    ...
    def _merge_courses(self, courses):
        ...
        for c in courses:
            key = (
                ...
                c.get("extra_info", {}).get("building", "")
                + c.get("extra_info", {}).get("classroom", ""),
            )
