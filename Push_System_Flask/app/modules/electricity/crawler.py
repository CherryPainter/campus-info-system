...
class ElectricityCrawler:
    ...
    def fetch_usage_records(self, max_pages: int | None = None) -> list[dict]:
        """
        分页爬取用电记录
        ...
        for page in range(1, pages + 1):
            try:
                ...
                if len(records) > 100:
                    logger.warning(
                        f"第 {page} 页返回 {len(records)} 条记录，数量异常，请检查解析逻辑"
                    )
        ...
        logger.info(
            f"fetch_usage_records: 共获取 {len(all_records)} 条记录（实际爬取 {max(last_page_with_data, 0)} 页，请求上限 {pages} 页）"
        )
        return all_records
