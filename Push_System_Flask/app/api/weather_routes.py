...
def status():
    ...
    # 判断模块是否启用
    jwt_configured = bool(current_app.config.get('QWEATHER_CREDENTIAL_ID')) and \
                      bool(current_app.config.get('QWEATHER_PROJECT_ID'))
    api_key_configured = bool(current_app.config.get('QWEATHER_API_KEY'))
    enabled = jwt_configured or api_key_configured

    return api_success(module='weather', enabled=enabled, api_key_configured=api_key_configured, jwt_configured=jwt_configured, config={'location': current_app.config.get('QWEATHER_LOCATION', '106.55,29.56'), 'city_name': current_app.config.get('QWEATHER_CITY_NAME', '重庆'), 'daily_push_time': current_app.config.get('WEATHER_SCHEDULE_DAILY', '07:00')}, cooldown={k: v for k, v in analyzer._cooldown_state.items()})
