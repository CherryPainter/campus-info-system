...
class Webhook(Base):
    ...
    modules = Column(String(100), default='course', comment='所属模块: course,weather,electricity,system 的逗号分隔列表')
    is_enabled = Column(Boolean, default=True, comment='是否启用')
    description = Column(String(500), nullable=True, comment='描述说明')
    last_test_status = Column(String(20), nullable=True, comment='上次测试结果: success/failed/pending')
    ...
    def get_enabled_webhooks(session, module=None):
        ...
        Returns:
            list: Webhook 对象列表
        """
        query = session.query(Webhook).filter(Webhook.is_enabled == True)
