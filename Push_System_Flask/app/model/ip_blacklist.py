...
class IPBlacklist(Base):
    ...
    source = Column(
        String(50),
        default="manual",
        comment="来源: manual手动/auto自动检测/ddos攻击检测/rate_limit限频",
    )
...
class IPSecurityEvent(Base):
    ...
    event_type = Column(
        ...
        nullable=False,
        index=True,
        comment="事件类型: rate_limit_exceeded/sql_injection/xss/suspicious_path/large_request/ddos/file_upload_abuse",
    )
    ...
    # 联合索引：快速查询某IP最近的事件
    __table_args__ = (Index("ix_ip_events_ip_time", "ip_address", "created_at"),)
