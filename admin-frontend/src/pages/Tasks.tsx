    ...
    tasks: [
      {
        ...
        icon: <SyncOutlined />,
        priority: "high",
        moduleOverride: "spider",
      },
      {
        key: "full_crawl",
        label: "全量爬取",
        desc: "全量爬取课程表（支持选择学期/周次）",
        icon: <SyncOutlined />,
        priority: "high",
      },
      {
        key: "push_daily_schedule",
        label: "推送今日课表",
        desc: "发送今日课程安排",
        icon: <BookOutlined />,
        priority: "normal",
...
// 假期模式下所有手动触发任务均应禁用（仅保留管理操作如 Cookie 检测可单独放行）。
const HOLIDAY_SKIPPED_KEYS = new Set<string>([
  // 爬虫：假期静默 or 非教学周 双条件拦截
  "spider",
  "full_crawl",
  // 课程推送：假期模式静默拦截（今日课表 / 周课表图片）
  "push_daily_schedule",
  "push_weekly_image",
  // 天气：所有天气任务在假期静默均被拦截
  "update_weather_now",
...
export default function Tasks() {
  ...
  const handleTrigger = async (
    taskType: string,
    module: "weather" | "electricity" | "spider" | "course"
  ) => {
  ...
  const renderTaskCard = (task: (typeof taskCategories)[0]["tasks"][0], module: string) => {
    ...
    const procRunning =
      !!proc && (proc.status === "running" || (proc.status as string) === "pending");
    const visualRunning = isRunning || procRunning || clickFlash.has(task.key);
    // 优先使用任务自身的 moduleOverride（如课程分类下的爬虫任务需走 spider 分支）
    const effectiveModule =
      "moduleOverride" in task && task.moduleOverride ? task.moduleOverride : module;
    // 假期模式禁用态：后端会拦截并返回 skipped，前端同步禁用 + 视觉反馈
    const isHolidayDisabled = !!holidayStatus?.active && HOLIDAY_SKIPPED_KEYS.has(task.key);
    ...
    if (task.key === "full_crawl") {
      ...
      const procRunning =
        !!proc && (proc.status === "running" || (proc.status as string) === "pending");
      const visualRunning =
        isBackendRunning("full_crawl") || procRunning || clickFlash.has("full_crawl");
      return (
        <Card
          hoverable
          style={{
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
            ...
            {visualRunning && <Badge status="processing" text="运行中" style={{ fontSize: 11 }} />}
          </div>
