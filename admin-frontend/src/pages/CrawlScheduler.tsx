...
import {
  Modal, Form, Select, Radio, Space, Alert, Tag, DatePicker
} from 'antd';
...
export default function CrawlScheduler({ visible, onClose, onStarted, isFullCrawl = false }: CrawlSchedulerProps) {
  ...
  return (
    ...
    >
      ...
      <Form form={form} layout="vertical" style={{ marginTop: 16 }} initialValues={{ scope: 'semester', schedule_type: 'immediate' }}>
        {/* 爬取范围 */}
        <Form.Item label="爬取范围" name="scope" rules={[{ required: true, message: '请选择爬取范围' }]}>
          <Radio.Group
            onChange={(e) => {
              if (e.target.value === 'semester' && selectedSemester === undefined && semesterList.length) {
