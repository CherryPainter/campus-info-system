...
export default function Weather() {
  ...
  const tabs = [
    {
      key: "now",
      label: <Space>实时天气{isPolling && <Badge dot offset={[4, -4]} />}</Space>,
      icon: <CloudOutlined />,
      children: (
        <div>
          {isPolling && (
              ...
              <Space style={{ marginTop: 16 }}>
                ...
                {isAdmin && (
                  <Button
                    ...
                    onClick={() => handleTrigger("update_weather_now")}
    ...
    tabs.push({
      ...
      children: loading ? (
        ...
        <Form form={form} layout="vertical" onFinish={handleSaveConfig} style={{ maxWidth: 600 }}>
          ...
          {/* API Key 配置（兼容旧版） */}
          <Form.Item name="api_key" label="API Key (兼容旧版)" tooltip="仅当不使用 JWT 时需要填写">
            <Input.Password placeholder="和风天气 API Key" />
          </Form.Item>

          <Form.Item name="location" label="位置坐标">
            <Input placeholder="经度,纬度 或 LocationID" />
          </Form.Item>
          <Form.Item name="city_name" label="城市名称">
          ...
          <Form.Item
            ...
            label="免打扰结束时间"
            tooltip="格式 HH:MM（24小时制），可跨午夜，如 23:00 - 07:00"
          >
            <Input placeholder="如：07:00" />
          </Form.Item>
          <Form.Item
