...
export default function Dashboard() {
  ...
  const typeTrend = processStats.type_trend || { dates: [] as string[], series: [] as { name: string; data: number[] }[] };
  ...
  const lineOption = {
    ...
    grid: {
      left: '3%', right: '4%', bottom: '12%', top: '6%', containLabel: true,
    },
    xAxis: {
      type: 'category',
  ...
  return (
    <div className="dashboard-container">
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card className="status-card" hoverable style={{ opacity: isOffline ? 0.5 : 1 }}>
            ...
            <Space split={<Divider type="vertical" />}>
              <Text type="secondary" style={{ fontSize: 12 }}>v{data?.system?.version || '-'}</Text>
              <Text type="secondary" style={{ fontSize: 12 }}>运行 {data?.system?.uptime || '-'}</Text>
            </Space>
          </Card>
        </Col>
          ...
          <Card className="status-card" hoverable>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ marginTop: 8 }}>
                  <Text strong style={{ fontSize: 20, color: STATUS_COLORS[data?.modules?.weather?.status || 'disabled'] || '#d9d9d9' }}>
                    {data?.modules?.weather?.enabled ? '正常' : '未启用'}
                  </Text>
                </div>
              </div>
              <CloudOutlined style={{ fontSize: 40, color: STATUS_COLORS[data?.modules?.weather?.status || 'disabled'] || '#d9d9d9' }} />
            </div>
            <Divider style={{ margin: '12px 0' }} />
            <Space size={4}>
          ...
          <Card className="status-card" hoverable>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ marginTop: 8 }}>
                  <Text strong style={{ fontSize: 20, color: STATUS_COLORS[data?.modules?.electricity?.status || 'disabled'] || '#d9d9d9' }}>
                    {data?.modules?.electricity?.enabled ? '正常' : '未启用'}
                  </Text>
                </div>
              </div>
              <ThunderboltOutlined style={{ fontSize: 40, color: STATUS_COLORS[data?.modules?.electricity?.status || 'disabled'] || '#d9d9d9' }} />
            </div>
            <Divider style={{ margin: '12px 0' }} />
            <Space split={<Divider type="vertical" />}>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card className="status-card" hoverable style={{ opacity: isOffline ? 0.5 : 1 }}>
            ...
            <Space size="small" style={{ fontSize: 12 }}>
              ...
              {data?.tasks?.spider_status?.course?.running ? <Tag color="processing">运行中</Tag> : <Tag>空闲</Tag>}
              <Text type="secondary">电量:</Text>
              {data?.tasks?.spider_status?.electricity?.running ? <Tag color="processing">运行中</Tag> : <Tag>空闲</Tag>}
            </Space>
          </Card>
        </Col>
      </Row>
      ...
      <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
          ...
          <Card style={{ height: '100%' }}>
            ...
            <div style={{
              ...
              <Space align="center">
                ...
                <Text strong style={{ fontSize: 16 }}>任务执行统计</Text>
                {timeLabel && (
                  <Tag color="processing" style={{ marginLeft: 4 }}>{timeLabel}</Tag>
                )}
              </Space>
            ...
            <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
              <Col xs={12} sm={6}>
                <Card size="small" styles={{ body: { padding: '14px 16px' } }}>
                  <Statistic
                    ...
                    value={timeRange === 'this_month' ? (processStats.today?.total || 0) : (period.total || 0)}
                    suffix="次"
              ...
              <Col xs={12} sm={6}>
                <Card size="small" styles={{ body: { padding: '14px 16px' } }}>
                  <Statistic
                    title={<Text type="secondary" style={{ fontSize: 13 }}>{timeRange === 'this_month' ? '今日完成' : '期间完成'}</Text>}
                    value={timeRange === 'this_month' ? (processStats.today?.completed || 0) : (period.completed || 0)}
                    suffix="次"
              ...
              <Col xs={12} sm={6}>
                <Card size="small" styles={{ body: { padding: '14px 16px' } }}>
                  <Statistic
                    title={<Text type="secondary" style={{ fontSize: 13 }}>{timeRange === 'this_month' ? '今日失败' : '期间失败'}</Text>}
                    value={timeRange === 'this_month' ? (processStats.today?.failed || 0) : (period.failed || 0)}
                    suffix="次"
              ...
              <Col xs={12} sm={6}>
                <Card size="small" styles={{ body: { padding: '14px 16px' } }}>
                  <Statistic
                    title={<Text type="secondary" style={{ fontSize: 13 }}>期间总数</Text>}
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              <Text strong style={{ fontSize: 14 }}>任务类型分布</Text>
              <Segmented
                size="small"
                value={chartType}
        ...
        <Col xs={24} lg={8}>
          <Card 
            title={<span><ClockCircleOutlined style={{ marginRight: 8 }} />最近任务</span>} 
      ...
      {/* 快捷操作 */}
      <Card 
        title={<span><ToolOutlined style={{ marginRight: 8 }} />快捷操作</span>} 
        style={{ marginTop: 16 }}
      >
        <Row gutter={[16, 16]}>
          <Col xs={12} sm={8} md={6}>
