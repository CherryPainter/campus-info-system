...
def _make_crawler():
    ...
    return ElectricityCrawler(
        ...
        max_pages=getattr(
            Config, "ELECTRICITY_CRAWLER_MAX_PAGES", 50
        ),  # 增加到50页以获取更多历史记录
    )
...
def _load_last_low_power_time() -> datetime | None:
...
def push_electricity_daily() -> None:
    ...
    try:
        ...
        if not success:
            _send_markdown(
                ElectricityFormatter.format_fetch_error("每日用电报告", msg), notify_only=False
            )
            complete_task_process(pid, "failed", error=msg)
            return
