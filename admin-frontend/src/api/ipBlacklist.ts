...
const ipBlacklistApi = {
  ...
  getList: async (params: { page?: number; per_page?: number; only_active?: boolean }) => {
    const res = await request.get<any, ApiResponse<BlacklistListResp>>("/admin/ip-blacklist", {
      params,
    });
  ...
  toggle: (ip: string, active: boolean) =>
    request.put<any, ApiResponse<IPBlacklistRecord>>(
      `/admin/ip-blacklist/${encodeURIComponent(ip)}/toggle`,
      { active }
    ),
  ...
  update: (
    ip: string,
    data: { reason?: string; duration_hours?: number | null; note?: string; is_active?: boolean }
  ) =>
    request.put<any, ApiResponse<IPBlacklistRecord>>(
      `/admin/ip-blacklist/${encodeURIComponent(ip)}/update`,
      data
    ),
  ...
  getEvents: async (params: {
    ...
    const res = await request.get<any, ApiResponse<SecurityEventListResp>>(
      "/admin/ip-blacklist/events",
      { params }
    );
    if (res.status !== "success") throw new Error(res.message || "请求失败");
