...
class TaskProcess(Base):
    ...
    task_type = Column(
        String(50),
        nullable=False,
        comment="任务类型: spider课表爬虫, course_full_crawl全量爬取, weather天气, electricity电量, course课表推送, custom自定义, system系统",
    )
    status = Column(
        String(20),
        default=TaskStatus.RUNNING,
        comment="状态: running运行中, completed已完成, completed_empty空成功, failed失败, cancelled已取消, pending待执行",
    )
