...
def log_redis_status() -> None:
    ...
    if not url:
        logger.info(
            "[Redis] 未配置 REDIS_URL，限流与登录爆破使用进程内存（开发/单机，重启会丢失计数）"
        )
        return
    try:
        ...
        logger.warning(
            f"[Redis] 连接失败（{exc}），限流与登录爆破降级为进程内存（仅本进程生效，多 worker 不共享）"
        )
...
def _match_tier(tiers: dict[int, dict], count: int) -> int | None:
...
class IPBlacklistService:
    ...
    def block_ip(
        ...
        """
        # 检查是否已在黑名单中
        existing = session.query(IPBlacklist).filter(IPBlacklist.ip_address == ip_address).first()
        ...
        session.commit()
        logger.warning(
            f"[IP黑名单] IP {ip_address} 已被加入黑名单 | 原因: {reason} | 来源: {source}"
        )
    ...
    def get_blacklist(
        ...
        if only_active:
            query = query.filter(IPBlacklist.is_active.is_(True))

        total = query.count()
        records = (
            query.order_by(desc(IPBlacklist.blocked_at))
    ...
    def get_security_events(
        ...
        query = session.query(IPSecurityEvent).filter(IPSecurityEvent.created_at >= since)
        ...
        total = query.count()
        events = (
            query.order_by(desc(IPSecurityEvent.created_at))
            .offset((page - 1) * per_page)
            .limit(per_page)
            .all()
        )
    ...
    def ignore_event(session: Session, event_id: int) -> bool:
        """将安全事件标记为已忽略（不再需要处置）"""
        event = session.query(IPSecurityEvent).filter(IPSecurityEvent.id == event_id).first()
        if not event:
            return False
    ...
    def record_event(
        ...
        should_block, block_reason, block_source, block_dur = IPBlacklistService._check_auto_block(
            session, ip_address
        )
        if should_block:
            IPBlacklistService.block_ip(
    ...
    @staticmethod
    def _check_auto_block(session: Session, ip_address: str) -> tuple[bool, str, str, int | None]:
        """检查是否需要自动封禁该IP
        ...
        recent_requests = (
            session.query(func.count(IPSecurityEvent.id))
            .filter(
                and_(
        ...
        violations = (
            ...
            .filter(
                and_(
                    IPSecurityEvent.ip_address == ip_address,
                    IPSecurityEvent.event_type.in_(
                        ["sql_injection", "xss", "suspicious_path", "path_traversal"]
                    ),
    ...
    @staticmethod
    def _send_block_alert(ip_address: str, reason: str, source: str, tier_info: dict | None = None):
    ...
    def cleanup_expired(session: Session) -> int:
        ...
        expired = (
            ...
            .filter(
                and_(
                    IPBlacklist.is_active.is_(True),
                    IPBlacklist.expires_at is not None,
        ...
        session.query(IPSecurityEvent).filter(IPSecurityEvent.created_at < cutoff).delete(
