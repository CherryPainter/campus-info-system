...
import { ThunderboltOutlined, BarChartOutlined, PieChartOutlined, ArrowUpOutlined, ArrowDownOutlined } from '@ant-design/icons';
...
export default function ElectricityChart() {
  ...
  const lineOption = {
    tooltip: { 
      ...
      formatter: (params: any) => `${params[0].axisValue}<br/>用电量: ${Number(params[0].value).toFixed(2)} 度` 
    },
    grid: { left: 50, right: 20, top: 40, bottom: 30 },
    xAxis: { type: 'category' as const, data: daily.map(d => d.date.slice(5)), axisLabel: { fontSize: 11 } },
  ...
  /** 柱状图：各电表用电对比 */
  const barOption = {
    tooltip: { trigger: 'axis' as const, formatter: (params: any) => `${params[0].name}<br/>用电量: ${params[0].value} 度` },
    grid: { left: 120, right: 20, top: 20, bottom: 30 },
    xAxis: { type: 'value' as const, name: '用电量(度)', nameTextStyle: { fontSize: 11 } },
    yAxis: { type: 'category' as const, data: by_meter.map(m => m.meter), axisLabel: { fontSize: 11 } },
    series: [{
      type: 'bar',
      data: by_meter.map((m, i) => ({ value: m.usage, itemStyle: { color: COLORS[i % COLORS.length], borderRadius: [0, 4, 4, 0] } })),
      barWidth: '50%',
    }],
  };
  ...
  /** 饼图：各电表用电占比 */
  const pieOption = {
    tooltip: { trigger: 'item' as const, formatter: (params: any) => `${params.name}<br/>${params.value} 度 (${params.percent}%)` },
    legend: { bottom: 0, textStyle: { fontSize: 11 } },
    series: [{
      type: 'pie',
      radius: ['40%', '65%'],
      center: ['50%', '45%'],
      data: by_meter.map((m, i) => ({ name: m.meter, value: m.usage, itemStyle: { color: COLORS[i % COLORS.length] } })),
      label: { formatter: '{b}\n{d}%', fontSize: 11 },
  ...
  return (
    <div>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
        <Col xs={12} sm={6}>
          <Card size="small">
            <Statistic title="总用电量" value={summary.total_usage} suffix="度" prefix={<ThunderboltOutlined />} valueStyle={{ color: '#1890ff' }} />
          </Card>
        </Col>
        <Col xs={12} sm={6}>
          <Card size="small">
            <Statistic title="日均用电" value={summary.avg_daily} suffix="度" prefix={<BarChartOutlined />} valueStyle={{ color: '#52c41a' }} />
          </Card>
        </Col>
        <Col xs={12} sm={6}>
          <Card size="small">
            <Statistic title="单日最高" value={summary.max_daily} suffix="度" prefix={<ArrowUpOutlined />} valueStyle={{ color: '#ff4d4f' }} />
          </Card>
        </Col>
        <Col xs={12} sm={6}>
          <Card size="small">
            <Statistic title="单日最低" value={summary.min_daily} suffix="度" prefix={<ArrowDownOutlined />} valueStyle={{ color: '#52c41a' }} />
          </Card>
        </Col>
      </Row>
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24}>
          <Card title="各电表用电对比" size="small">
            <ReactECharts option={barOption} style={{ height: Math.max(200, by_meter.length * 50) }} />
          </Card>
        </Col>
      </Row>
    </div>
