...
import { Card, Tabs, Statistic, Row, Col, Alert, Form, Input, Button, Spin, Progress, Popconfirm, Space, Badge, App, Tag } from 'antd';
import { useRunningTasksPolling } from '@/hooks/useRunningTasksPolling';
import { useTaskPolling } from '@/hooks/useTaskPolling';
import ResponsiveTable from '@/components/ResponsiveTable';
import { ThunderboltOutlined, ReloadOutlined, PlayCircleOutlined, SettingOutlined, LineChartOutlined, DeleteOutlined, CloudDownloadOutlined, LoadingOutlined } from '@ant-design/icons';
import { adminApi, processApi, type TaskProcess } from '@/api/admin';
import { electricityApi, type ElectricityRemaining, type ElectricityRecord } from '@/api/electricity';
...
export default function Electricity() {
  ...
  const fetchRemaining = async () => {
    ...
    } catch (error) { console.error('加载剩余电量失败:', error); }
  ...
  const fetchRecords = async () => {
    ...
    } catch (error) { console.error('加载用电记录失败:', error); }
    finally { setLoading(false); }
  };
  ...
  const handleFetchAll = async () => {
    ...
    } catch (error) { message.error('全量爬取触发失败'); }
  };
  ...
  const recordColumns = [
    ...
    { title: '用电量', dataIndex: 'usage', key: 'usage', width: '22%', ellipsis: true, render: (v: number) => v != null ? `${v} 度` : '-' },
  ...
  const tabs = [
    {
      key: 'remaining', label: <Space>剩余电量{isPolling && <Badge dot offset={[4, -4]} />}</Space>, icon: <ThunderboltOutlined />,
      children: (
        <div>
          {isPolling && (
            <Alert
          ...
          {loading ? <Spin /> : remaining ? (
            <div>
              ...
              <Space style={{ marginTop: 16 }}>
                <Button icon={<ReloadOutlined />} onClick={fetchRemaining} disabled={isPolling}>刷新数据</Button>
                {isAdmin && <Button type="primary" icon={<PlayCircleOutlined />} onClick={() => handleTrigger('fetch_electricity_data')} disabled={isPolling} loading={isPolling}>触发数据采集</Button>}
              </Space>
            </div>
          ) : (
            <div style={{ textAlign: 'center', padding: 40 }}>
              <Alert message="暂无数据，请先触发数据采集" type="info" />
              {isAdmin && <Button type="primary" icon={<PlayCircleOutlined />} onClick={() => handleTrigger('fetch_electricity_data')} style={{ marginTop: 16 }} disabled={isPolling} loading={isPolling}>触发数据采集</Button>}
            </div>
          )}
        </div>
      ),
    ...
    {
      key: 'records', label: <Space>用电记录{isPolling && <Badge dot offset={[4, -4]} />}</Space>,
      children: (
        <div>
          {isPolling && (
            <Alert
          ...
          {loading ? <Spin /> : (
            <div>
              <Space style={{ marginBottom: 16 }}>
                <Button icon={<ReloadOutlined />} onClick={fetchRecords} disabled={isPolling}>刷新</Button>
                {isAdmin && (
                  <>
                    <Button icon={<CloudDownloadOutlined />} onClick={handleFetchAll} disabled={isPolling}>全量爬取</Button>
                    <Popconfirm
                      title="确定要删除全部用电记录吗？"
                    ...
                    >
                      <Button danger icon={<DeleteOutlined />} disabled={isPolling}>清空全部记录</Button>
                    </Popconfirm>
                  </>
                )}
              </Space>
              <ResponsiveTable dataSource={recordsWithKeys} columns={recordColumns} rowKey="_uid" mobileNativeTable tableLayout="fixed" pagination={{ pageSize, pageSizeOptions: ['10', '20', '50'], showSizeChanger: true, onShowSizeChange: (_current, size) => setPageSize(size) }} size="small" />
            </div>
          )}
        </div>
      ),
  ...
  if (isAdmin) {
    tabs.push({
      key: 'config', label: '模块配置', icon: <SettingOutlined />,
      children: loading ? <Spin /> : (
        <Form form={form} layout="vertical" onFinish={handleSaveConfig} style={{ maxWidth: 600 }}>
          <Form.Item name="cookie" label="爬虫 Cookie"><Input.TextArea rows={3} placeholder="JSESSIONID=xxx; leech_k=xxx" /></Form.Item>
          <Form.Item name="low_power_threshold" label="低电量阈值"><Input type="number" placeholder="如：10" suffix="度" /></Form.Item>
          <Form.Item name="daily_push_time" label="每日推送时间"><Input placeholder="如：00:30" /></Form.Item>
          <Form.Item name="weekly_push_day" label="每周推送日"><Input placeholder="如：mon" /></Form.Item>
