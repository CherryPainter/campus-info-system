...
from sqlalchemy import DECIMAL, Boolean, Column, DateTime, Index, Integer, String, TypeDecorator
...
class Course(Base):
    ...
    periods = Column(
        ...
        nullable=True,
        comment="所有节次，JSON数组格式（如：[1,2] 或 [5,6,7,8]）",
    )
    ...
    weeks = Column(
        MutableList.as_mutable(JSONEncodedList()),
        nullable=True,
        comment="上课周次，JSON数组格式（如：[1,2,3] 或 [1,3,5]）",
    )
    weeks_bitmap = Column(
        String(30), nullable=True, comment="周次位图（如：111100...，用于快速查询）"
    )
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "last_verified_at": self.last_verified_at.strftime("%Y-%m-%d %H:%M:%S")
